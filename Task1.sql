...
EXEC GetDiagnosis_And_AllergiesFor_PatientToday @PatientID = 2;
...
CREATE PROCEDURE Registration_of_Patient
...
BEGIN
  ...
  -- Check if the username already exists --
    ...
    VALUES (@FirstName, @MiddleName, @LastName, @DateOfBirth, @Gender, @NHSNumber, @Address, @City, @State, @PostCode, @TelephoneNumber, @Email, @Username, @Password);
    SELECT 'Patient record inserted successfully' AS [Message];
END

-- TO CALL THE PROCEDURE --

EXEC Registration_of_Patient
    @FirstName = 'Kevin',
    @MiddleName = 'David',
    @LastName = 'Doe',
    ...
    @Username = 'Kevindoe896',
    @Password = 'Kevindoe@356';
  SELECT * FROM PatientRecords;

-- CREATE PROCEDURE TO BOOK/REBOOK APPOINTMENT --

CREATE PROCEDURE _BookAppointment_
    @Username NVARCHAR(60),
    @Password NVARCHAR(60),
    @DoctorID INT,
...
BEGIN
    ...
    SELECT @PatientID = PatientID FROM PatientRecords 
...
EXEC _BookAppointment_
    @Username = 'Natalie789',
    @Password = 'Williams@753',
    @DoctorID = 801,  
    @AppointmentDate = '2024-05-20',
    @AppointmentTime = '16:00:00',
    @Department = 'Gastroenterologists';
	SELECT * FROM PatientAppointments WHERE PatientID = '2';
...
CREATE LOGIN Anna WITH PASSWORD = 'Salford123';
CREATE USER Anna FOR LOGIN Anna;
CREATE ROLE HospitalAdministrator;

ALTER ROLE HospitalAdministrator ADD MEMBER Anna;
GRANT SELECT,INSERT,UPDATE ON  MedicalRecords TO Anna WITH GRANT OPTION;
GRANT SELECT,INSERT,UPDATE ON  MedicalRecords TO HospitalAdministrator WITH GRANT OPTION;
REVOKE DELETE ON MedicalRecords FROM Anna;
...
BACKUP DATABASE Hospital_Task_1 TO DISK = 'E:\00755216backup\00755216_task1.bak'
WITH CHECKSUM;
