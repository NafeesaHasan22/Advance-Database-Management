...
AND EXISTS (
    ...
    WHERE R.Restaurant_id = RC.Restaurant_id AND RC.Cuisine = 'Mexican'
);
...
SELECT R.Name AS Restaurant_Name, R.City, R.State, R.Country, R.Zip_code, R.Latitude, R.Longitude, Ra.Food_Rating
FROM Restaurants R
JOIN Ratings Ra ON R.Restaurant_id = Ra.Restaurant_id
JOIN (
...
) AS MinAges ON Ra.Consumer_id = MinAges.Consumer_id 
AND MinAges.Min_Age = (SELECT Age FROM Consumers WHERE Consumer_id = Ra.Consumer_id)
